...
cy_propertyAttributes * cy_copyPropertyAttributes (objc_property_t property) {
    ...
    cy_propertyAttributes * attributes = calloc(1, sizeof(cy_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate cy_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && * next == ',') {
        ...
        switch (flag) {
            ...
            case 'R':
                ...
                break;
            case 'C':
                attributes->memoryManagementPolicy = cy_propertyMemoryManagementPolicyCopy;
                break;
            case '&':
                attributes->memoryManagementPolicy = cy_propertyMemoryManagementPolicyRetain;
                break;
            case 'N':
