...
/// 上传数据 fileDatas: 要上传的文件数据, name: 服务器的资源文件名, mimeType: 文件资源类型, 传 nil 默认为 application/octet-stream
        ...
        if ([responseObject isKindOfClass:NSArray.class]) {
            ...
            if (resultClass == nil) {
                ...
                if ([jsonArr isKindOfClass:NSArray.class]) {
                    /// 字典数组转模型
                    NSArray * parsedObjects = [CYDataModel modelArrayWithJSON:jsonArr];
                    /// 解析是否 CYDataModel 子类
                    for (id parsedObject in parsedObjects) {
                        /// 确保解析出来的类为 CYDataModel
                        NSAssert([parsedObject isKindOfClass:CYDataModel.class], @"Parsed model object is not an CYDataModel: %@", parsedObject);
                    }
                    [subscriber sendNext:parsedObjects];
                } else {
